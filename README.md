...
3. 色一覧から、テーマに追従させたい色をクリックしてcurrentColor化する
4. 3背景のプレビューで見え方を確かめ、整形済みコードをコピーするかSVGファイルとしてダウンロードする

入力内容と各オプションはブラウザに保存され、次回そのまま開けます。

### 変換の内訳

| 処理               | 内容                                                                                         |
| :----------------- | :------------------------------------------------------------------------------------------- |
| 最適化             | コメント、metadata、inkscape / sodipodi等のエディタ名前空間を除去し、パスの小数を指定桁(既定2桁)に丸める |
...
変換はすべてブラウザ内のDOM操作で行われ、SVGはどこにも送信されません。アニメーションSVGやscript入りSVGの動作は保証しません。
